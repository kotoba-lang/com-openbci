(ns openbci.main
  "Kotodama WASM entrypoint for the Openbci clean-room actor (L4) — Clojure port.
  ...
  facts (`openbci.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "openbci")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"openbci.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "openbci-compat" :tier tier :entities entities} 200])
...
(defn start! [] :openbci-compat/ready)
